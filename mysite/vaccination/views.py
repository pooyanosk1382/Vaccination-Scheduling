...
class ConfirmVaccination(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            if is_reserved:
                form.save()
                return HttpResponse("Your vaccination has beem scheduled")
...
def appointment_letter(request, vaccination_id):
    ...
    context = {
        ...
        "content": f"This os to inform that the {vaccination.campaign.vaccine.name} vaccination of Mr/Ms {vaccination.get_full_name()} is scheduled on {vaccination.slot.date}",
    }
...
def vaccination_certificate(request, vaccination_id):
    ...
    if vaccination.is_vaccinated:
        context = {
            "pdf_title": f"{vaccination.patient.get_full_name()} | Vaccination Certified",
            "date": str(timezone.now()),
